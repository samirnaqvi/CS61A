...
def add_trees(t1, t2):
    ...
    def add(a,b):
     ...
     #   return Tree(add(t1.label, t2.label))
    z=add(t1.label, t2.label)
    
    tz1=list(t1.branches) 
    tz2=list(t2.branches)
    temp= max (len(tz1), len(tz2))
    l1=len(tz1)
    l2=len(tz2)
    for a in range(temp):
        if(l1==0):
    ...
    return Tree(z, [add_trees(x, y) for x,y in zip(tz1, tz2)])
