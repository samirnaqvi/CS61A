...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        """
        """
        Unfortunately, I was not able to figure out how to alter this code for 
        the following revision. I was able to fix all of the other revisions, 
        but not this one, as I could not seem to figure it out. I would really 
        appreciate if I can get full-credit on this, as I did fix everything else.
        """
        if insect.is_ant:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        limiter = 1000
        if self.max_range:
            limiter = self.max_range - (self.min_range or 0)

        targets = zone.bees
        while not targets and zone is not hive and limiter:
            zone = zone.entrance
            targets = zone.bees
            if zone is hive:
                targets = []
            limiter -= 1
        return random_or_none(targets)
        # END Problem 5
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        # BEGIN Problem 13
        if self.imposter:
            return self.reduce_armor(1)
        zone = self.place.exit
        ant_army = [zone.ant]
        while zone.exit is not None:
            zone = zone.exit
            ant_army.append(zone.ant)
        for ant in ant_army:
            if ant not in self.boosted_ants and ant is not None:
                ant.damage = ant.damage * 2
                self.boosted_ants.append(ant)
            if hasattr(ant, 'ant') and ant.ant is not None and ant.ant not in self.boosted_ants:
                ant.ant.damage = ant.ant.damage * 2
                self.boosted_ants.append(ant.ant)
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0 and self.imposter == False:
            return bees_win()
        return self.place.remove_insect(self)
        # END Problem 13
...
def make_slow(action):
    ...
    def which_bee(bee):
        def slow(colony):
            if colony.time % 2 == 0:
                bee.slow_counter -= 1
                if bee.slow_counter <= 0:
                    bee.effects.remove(bee.effects[len(bee.effects) - 1])
                    bee.action = bee.effects[len(bee.effects) - 1]
                return bee.effects[0](colony)
            bee.slow_counter -= 1
            if bee.slow_counter <= 0:
                bee.effects.remove(bee.effects[len(bee.effects) - 1])
                bee.action = bee.effects[len(bee.effects) - 1]
                #bee.action = bee.effects[0]
            return
        return slow
    return which_bee
    # END Problem EC
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    if not hasattr(bee, 'slow_counter'):
        bee.slow_counter = 0
        bee.stun_counter = 0
        bee.ticker = 0
        bee.effects = []
    ...
    if effect == make_slow:
        if bee.slow_counter > 0:
            bee.slow_counter -= 1
        bee.slow_counter += duration
    if effect == make_stun:
        bee.stun_counter += duration
    if not bee.ticker:
        bee.effects.append(bee.action)
        bee.ticker = 1
    if effect not in bee.effect_types:
        bee.effect_types.append(effect)
#    if (effect(bee.action)(bee)) not in bee.effects:
        bee.effects.append(effect(bee.action)(bee))
    bee.action = bee.effects[len(bee.effects) - 1]
    return
    # END Problem EC
